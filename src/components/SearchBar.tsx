...
export default function SearchBar({ setCity }: SearchBarProps){
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        fetch(`http://api.openweathermap.org/geo/1.0/direct?q=${debounceCheck}&limit=20&appid=34f1a0bb9f6e351772d63ee557da9c6e`)
        .then((res) => res.json())
        .then((res) => {
            setSuggestions(res);
    ...
    return (
        <section className="w-full max-w-4xl mx-auto px-4 py-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-stretch sm:items-center" ref={wrapperRef}>
                ...
                <div className="relative flex-1">
                    ...
                    {showSuggestions && suggestions.length > 0 && (
                        <div className="absolute top-full left-0 right-0 mt-2 z-50">
                            <ul className="w-full bg-white dark:bg-gray-800 
                                           border-2 border-purple-300 dark:border-purple-700
                                           rounded-lg shadow-2xl overflow-hidden
                                           max-h-96 overflow-y-auto
                                           backdrop-blur-sm">
                                <li className="px-4 py-2 bg-purple-100 dark:bg-purple-900/50 
                    ...
                    {showSuggestions && !isLoading && searchVal.trim().length >= 2 && suggestions.length === 0 && (
                        <div className="absolute top-full left-0 right-0 mt-2 z-50">
                ...
                <Favourites setCity={setCity} button={
                    <ButtonsForSearchBarComponent
                        icon={BookMarked}
                        variant="warning"
                    >
